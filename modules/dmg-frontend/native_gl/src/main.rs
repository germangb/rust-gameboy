...
fn main() {
    let mut ppu = PPUUi { display: true,
                          palette: true,
                          tiles: false,
                          background: false,
                          window: false,
    ...
    let window = video.window("DMG - GL", 800, 600)
                      .opengl()
                      .position_centered()
                      //.fullscreen()
                      .build()
    ...
    let mut emulator = Builder::default()
        .cartridge(cartridge::from_bytes(include_bytes!("../../native/roms/tetris.gb")).expect("Error creating cartridge"))
        .video(GLVideo::new(Lcd::default()))
        .gb_mode()
        .build();
    ...
    loop {
        ...
        if ppu.display {
            imgui::Window::new(imgui::im_str!("Display")).always_auto_resize(true)
                                                         .resizable(false)
                                                         .build(&ui, || {
                                                             let texture = emulator.mmu()
                                                                                   .ppu()
                                                                                   .video()
                                                                                   .texture();
                                                             let scale = 3.0;
...
fn handle_input(event_pump: &mut EventPump,

...
fn check_logo(rom: &[u8]) -> TestResult {
    ...
    if let Some((i, (a, b))) = fail {
        TestResult::Err(format!(
            "The logo comparison failed at the {}th byte ({:02x} != {:02x})",
            i, a, b
        ))
    } else {
        TestResult::Ok
...
// at 0x014d in the rom header. If the values don't match, the rom won't boot.
fn check_header_checksum(bytes: &[u8]) -> TestResult {
    let x = bytes[0x134..=0x14c]
        .iter()
...
fn check_rom(rom: &[u8]) -> Result<Test, Test> {
    ...
    Test {
        title: rom.iter().skip(0x134).take(TITLE_LEN).copied().collect(),
        ...
        global_checksum: check_global_checksum(rom),
    }
    .into_result()
}
